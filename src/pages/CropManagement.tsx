...
export default function CropManagement() {
    ...
    const setCrops = useCropStore(state => state.setCrops)
    const removeCrop = useCropStore(state => state.removeCrop)
    const {control, handleSubmit, reset, formState: {errors}} = useForm<Crop>()

    const onSubmit = handleSubmit(data => {
        addCrop({...data, id: crops.length ? crops[crops.length - 1].id + 1 : 0})
        reset()
    })
                            ...
                            <Table striped bordered hover>
                                ...
                                {crops.map((crop) => (
                                    <tr key={crop.id}>
                                        ...
                                        <td>{crop.type}</td>
                                        <td>{crop.date}</td>
                                        <td><Button onClick={() => removeCrop(crop.id)} variant="danger" size="sm">Remove</Button></td>
                                    </tr>
                                ))}
                                </tbody>
                            </Table>
