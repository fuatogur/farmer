...
const useFertilizationStore = create<State>()(persist((set) => ({
    fertilizations: [],
    addFertilization: (fertilization) => set((state) => ({fertilizations: [...state.fertilizations, fertilization]})),
    setFertilizations: (fertilizations) => set({fertilizations}),
    removeFertilization: (id) => set((state) => ({fertilizations: state.fertilizations.filter(fertilization => fertilization.id !== id)}))
}), {
