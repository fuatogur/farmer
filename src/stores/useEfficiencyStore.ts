...
const useEfficiencyStore = create<State>()(persist((set) => ({
    efficiencies: [],
    addEfficiency: (efficiency) => set((state) => ({efficiencies: [...state.efficiencies, efficiency]})),
    setEfficiencies: (efficiencies) => set({efficiencies}),
    removeEfficiency: (id) => set((state) => ({efficiencies: state.efficiencies.filter(efficiency => efficiency.id !== id)}))
}), {
